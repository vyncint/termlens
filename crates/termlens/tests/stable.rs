...
///
/// The bell loop is one write per iteration and nothing else — no `sleep`,
/// and no process spawned per iteration, which used to stall for hundreds
/// of milliseconds on a loaded machine, and a stall is real silence; the
/// stress workflow found exactly that against a shell loop. The
