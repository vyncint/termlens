...
**Commits**: 20 on `main` (21 with this report), every one Conventional,
DCO-signed-off by `Vyncint Ng <vyncint@icloud.com>`, zero AI attribution —
verified by running both policy scripts over the full history locally *and*
by the `commit-policy` runs on every push.
